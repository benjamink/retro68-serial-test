...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        -h|--help)
            ...
            echo "  -d, --dc42        Also create Disk Copy 4.2 image (SerialSend.dc42)"
            exit 0
            ;;
        *)
...
dd if=/dev/zero of=SerialSend.img bs=1024 count=800 2>/dev/null
...
if [ "$MAKE_DC42" -eq 1 ]; then
    python3 "$(dirname "$0")/make_dc42.py" SerialSend.img SerialSend.dc42 "Serial Send"
fi
...
if [ "$BUILD_ONLY" -eq 1 ]; then
    echo "Build complete. Disk image: SerialSend.img"
    [ "$MAKE_DC42" -eq 1 ] && echo "Disk Copy 4.2 image: SerialSend.dc42"
    exit 0
fi
